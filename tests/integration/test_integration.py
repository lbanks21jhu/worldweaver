...
def test_integration():
    ...
    storylets_without_coords = (
        db.query(Storylet)
        .filter((Storylet.spatial_x.is_(None)) | (Storylet.spatial_y.is_(None)))
        .limit(3)
        .all()
    )
    ...
    if storylets_without_coords:
        ...
        print(
            f"   Found {len(storylet_ids)} storylets without coordinates: {storylet_ids}"
        )
        ...
        try:
            ...
            if location:
                ...
                if storylet.spatial_x is not None and storylet.spatial_y is not None:
                    ...
                    print(
                        f"   📍 {storylet.title[:30]:30} | {location:20} | ({storylet.spatial_x:3}, {storylet.spatial_y:3})"
                    )
                else:
                    print(
                        f"   ❌ {storylet.title[:30]:30} | {location:20} | No coordinates"
                    )
        except:
            continue
    ...
    print(
        f"   Coverage: {100 * coordinates_assigned / locations_found if locations_found > 0 else 0:.1f}%"
    )
    ...
    if coordinates_assigned == locations_found:
        ...
        print(
            f"⚠️ {locations_found - coordinates_assigned} storylets still need coordinates"
        )
