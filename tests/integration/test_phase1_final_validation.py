...
def test_all_fixes():
    ...
    manager.update_environment(
        {"time_of_day": "dawn", "weather": "clear", "danger_level": 3}
    )
    ...
    assert (
        final_context["relationship_count"] == 3
    )  # We have ally, rival, and friend from earlier test
    ...
    assert (
        final_summary["stats"]["total_relationships"] == 3
