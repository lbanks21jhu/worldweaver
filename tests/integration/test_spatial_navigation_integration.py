...
async def test_eight_direction_movement():
    ...
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        for direction in directions:
            response = await client.post(f"/api/spatial/move/{session_id}", json={"direction": direction})
        ...
        response = await client.post(f"/api/spatial/move/{session_id}", json={"direction": "N"})
        data = response.json()
