...
def test_advanced_scenarios():
    ...
    manager.add_item(
        "quantum_crystal",
        "Quantum Resonance Crystal",
        1,
        {"power": 15, "stability": 0.9, "dimensional": True, "ancient_artifact": True},
    )
    manager.add_item(
        "star_compass", "Stellar Navigation Compass", 1, {"range": 8, "charge": 0.7}
    )
    ...
    print(
        f"   Energy: {manager.variables['energy']}, Resonance: {manager.variables['resonance_level']}"
    )
    ...
    keeper_rel = manager.update_relationship(
        "player", "observatory_keeper", {"trust": 0.3, "respect": 0.6}
    )
    keeper_rel.add_memory("First meeting - demonstrated quantum manipulation")

    # Meet a rival reality-shaper
    rival_rel = manager.update_relationship(
        "player", "shadow_weaver", {"trust": -0.2, "respect": 0.1}
    )
    rival_rel.add_memory("Caught distorting local reality without permission")

    # Meet an ancient being
    ancient_rel = manager.update_relationship(
        "player", "void_sage", {"trust": 0.8, "respect": 0.9}
    )
    ...
    for rel_key, rel in manager.relationships.items():
        print(
            f"   {rel_key}: {rel.get_overall_disposition()} (trust: {rel.trust:.1f}, respect: {rel.respect:.1f})"
        )
    ...
    print(
        f"✅ Environment: {context['_weather']} {context['_time_of_day']}, danger: {context['_danger_level']}"
    )
    ...
    reality_conditions = {"item:quantum_crystal": {"quantity": {"gte": 1}}}
    ...
    print(
        f"✅ Items: {summary['inventory_summary']['total_items']} types, {summary['inventory_summary']['total_quantity']} total"
    )
    ...
    avg_reputation = sum(
        rel.trust + rel.respect for rel in manager.relationships.values()
    ) / len(manager.relationships)
