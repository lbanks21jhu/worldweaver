...
def test_database_is_empty():
    ...
    try:
        ...
        if storylet_count == 0 and session_count == 0:
            ...
            print(
                "❌ Database contains data; this is informational in non-fresh environments."
            )
            pytest.skip(
...
def test_database_tables_exist():
    ...
    try:
        ...
        if missing_session_cols:
            print(
                f"❌ FAIL: session_vars table missing columns: {missing_session_cols}"
            )
...
def test_database_can_insert():
    ...
    try:
        ...
        assert (
            remaining == 0
        ), f"Test record not cleaned up; remaining count: {remaining}"
