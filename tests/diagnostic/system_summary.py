...
def test_system_status():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"✅ Analysis System: {summary.get('connectivity_health', 0):.1%} connectivity health"
            )
    ...
    try:
        ...
        response = requests.post(
            "http://localhost:8000/author/generate-intelligent", json=payload
        )
        if response.status_code == 200:
            data = response.json()
            if "error" in data and "No storylets generated" in data["error"]:
                print(
                    "⚠️  Intelligent Generation: Working but no API key (using fallbacks)"
                )
            elif "storylets" in data:
                print(
                    f"✅ Intelligent Generation: Created {len(data['storylets'])} storylets"
                )
            else:
                print(
                    f"⚠️  Intelligent Generation: {data.get('message', 'Unknown status')}"
                )
        else:
