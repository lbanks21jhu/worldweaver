...
class TestDatabaseEnvironmentLogic:
    ...
    @patch.dict(
        os.environ, {"DW_DB_PATH": "/absolute/path/to/database.db"}, clear=False
    )
    ...
    @patch.dict(
        os.environ,
        {"DW_DB_PATH": "priority_test.db", "PYTEST_CURRENT_TEST": "test_something"},
        clear=False,
    )
    ...
    def test_multiple_environment_scenarios(self):
        ...
        for dw_db_path, pytest_test, expected_db in test_scenarios:
            ...
            with patch.dict(os.environ, env_vars, clear=True):
                ...
                assert (
                    db_file == expected_db
                ), f"Failed for DW_DB_PATH={dw_db_path}, PYTEST={pytest_test}"
