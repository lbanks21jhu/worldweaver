...
def test_author_commit_skips_duplicates():
    ...
    try:
        ...
        session.query(Storylet).filter(
            Storylet.title == "Unique Duplicate Test"
        ).delete()
        session.commit()
        ...
        rows = (
            session.query(Storylet)
            .filter(Storylet.title == "Unique Duplicate Test")
            .all()
        )
        assert len(rows) == 1
        ...
        # Clean up
        session.query(Storylet).filter(
            Storylet.title.in_(["Unique Duplicate Test", "Another Title"])
        ).delete()
        session.commit()
        session.close()
