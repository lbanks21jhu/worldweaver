...
class TestAuthorInputValidation:
    ...
    def test_suggest_req_valid_n_parameter(self):
        ...
        valid_requests = [SuggestReq(n=1), SuggestReq(n=10), SuggestReq(n=20)]
    ...
    def test_populate_endpoint_target_count_validation(self):
        ...
        for target_count in [1, 50, 100]:
            try:
                response = requests.post(
                    f"{BASE_URL}/author/populate?target_count={target_count}", timeout=3
                )
        ...
        assert all(
            code != 400 for code in valid_responses
        ), "Valid target_count values should not return 400"
        # Test invalid target_count values - too small
        response = requests.post(
            f"{BASE_URL}/author/populate?target_count=0", timeout=3
        )
        assert response.status_code == 400
        error_detail = response.json()["detail"]
        assert "target_count must be at least 1" in error_detail
        response = requests.post(
            f"{BASE_URL}/author/populate?target_count=-5", timeout=3
        )
        assert response.status_code == 400
        error_detail = response.json()["detail"]
        assert "target_count must be at least 1" in error_detail
        ...
        response = requests.post(
            f"{BASE_URL}/author/populate?target_count=150", timeout=3
        )
        assert response.status_code == 400
        error_detail = response.json()["detail"]
        assert "target_count cannot exceed 100" in error_detail
        response = requests.post(
            f"{BASE_URL}/author/populate?target_count=500", timeout=3
        )
        assert response.status_code == 400
        error_detail = response.json()["detail"]
        assert "target_count cannot exceed 100" in error_detail
    ...
    def test_suggest_endpoint_with_invalid_n(self):
        ...
        invalid_payload = {"n": 25, "themes": ["adventure"], "bible": {}}  # Too large
        try:
            response = requests.post(
                f"{BASE_URL}/author/suggest", json=invalid_payload, timeout=3
            )
        except requests.ConnectionError:
            pytest.fail(
                "Server is not running at http://localhost:8000. Please start the FastAPI server before running this test."
