    ...
    with SessionLocal() as db:
        ...
        for i, storylet in enumerate(storylets):
            ...
            if any(word in title_lower for word in ["abyss", "void", "dark", "shadow"]):
                ...
                word in title_lower for word in ["forest", "tree", "green", "nature"]
            ):
                location = "Deep Forest"
            elif any(
                word in title_lower for word in ["mountain", "peak", "high", "summit"]
            ):
                location = "Mountain Peak"
            elif any(
                word in title_lower
                for word in ["crystal", "gem", "cave", "underground"]
            ):
                location = "Crystal Cave"
            elif any(
                word in title_lower for word in ["ancient", "old", "ruin", "past"]
            ):
                location = "Ancient Ruins"
            elif any(
                word in title_lower for word in ["mystic", "magic", "grove", "sacred"]
            ):
                location = "Mystic Grove"
            elif any(word in title_lower for word in ["shadow", "shade", "phantom"]):
                location = "Shadow Realm"
            elif any(
                word in title_lower for word in ["light", "temple", "holy", "divine"]
            ):
        ...
        for source_loc, target_loc, choice_label in choice_connections:
            ...
            source_storylets = [
                s for s in storylets if location_assignments.get(s.id) == source_loc
            ]
            ...
            if source_storylets:
                ...
                new_choice = {"label": choice_label, "set": {"location": target_loc}}
                ...
                if not any(c.get("label") == choice_label for c in current_choices):
                    ...
                    print(
                        f"  Added choice: {storylet.title[:30]}... -> '{choice_label}' -> {target_loc}"
                    )
