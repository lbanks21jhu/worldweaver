...
def debug_database_state():
    ...
    db = next(get_db())
    result = db.execute(
        text("SELECT id, title, spatial_x, spatial_y, requires FROM storylets")
    )
    ...
    for row in result.fetchall():
        ...
        print(
            f"  ID {id_val:2}: {title[:30]:30} | Coords: {coords_str:8} | Location: {location}"
        )
    ...
    if storylets_with_locations > 0 and storylets_with_coords == 0:
        ...
        print(
            f"    This suggests spatial assignment didn't run properly during world generation."
        )
